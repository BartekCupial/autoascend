...
class MuZero:
    ...
    def __init__(self, game_name, config=None, split_resources_in=1):
        ...
        if self.config.max_num_gpus == 0 and (
            self.config.selfplay_on_gpu or self.config.train_on_gpu or self.config.reanalyse_on_gpu
        ):
            raise ValueError(
        ...
        if self.config.selfplay_on_gpu or self.config.train_on_gpu or self.config.reanalyse_on_gpu:
            total_gpus = self.config.max_num_gpus if self.config.max_num_gpus is not None else torch.cuda.device_count()
        else:
            total_gpus = 0
    ...
    def train(self, log_in_tensorboard=True):
        ...
        # Launch workers
        [
            self_play_worker.continuous_self_play.remote(self.shared_storage_worker, self.replay_buffer_worker)
    ...
    def load_model(self, checkpoint_path=None, replay_buffer_path=None):
        ...
        if replay_buffer_path:
            if os.path.exists(replay_buffer_path):
                ...
                self.checkpoint["num_played_steps"] = replay_buffer_infos["num_played_steps"]
...
def hyperparameter_search(game_name, parametrization, budget, parallel_experiments, num_tests):
    ...
    """
    optimizer = nevergrad.optimizers.OnePlusOne(parametrization=parametrization, budget=budget)
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        games = [
            filename[:-3]
            for filename in sorted(os.listdir(os.path.dirname(os.path.realpath(__file__)) + "/games"))
