...
class ReplayBuffer:
        ...
        if self.config.PER:
            if game_history.priorities is not None:
                ...
                for i, root_value in enumerate(game_history.root_values):
                    priority = (
                        numpy.abs(root_value - self.compute_target_value(game_history, i)) ** self.config.PER_alpha
                    )
    ...
    def compute_target_value(self, game_history, index):
        ...
        for i, reward in enumerate(game_history.reward_history[index + 1 : bootstrap_index + 1]):
            ...
            value += (
                reward if game_history.to_play_history[index] == game_history.to_play_history[index + i] else -reward
