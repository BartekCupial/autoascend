            ...
            for dx in [-1, 0, 1]:
                ...
                if 0 <= py < walkable.shape[0] and 0 <= px < walkable.shape[1] and (dy != 0 or dx != 0):
                    if walkable[py, px] and (
                        abs(dy) + abs(dx) <= 1
                        or (
                            walkable_diagonally[py, px]
                            and walkable_diagonally[y, x]
                            and (can_squeeze or walkable[py, x] or walkable[y, px])
                        )
                    ):
                        if dis[py, px] == -1:
                            dis[py, px] = dis[y, x] + 1
...
def slice_square_with_padding(array, center_y, center_x, radius, pad_value=0):
    return slice_with_padding(
        array, center_y - radius, center_y + radius + 1, center_x - radius, center_x + radius + 1, pad_value=pad_value
    )
...
def plot_dashboard(fig, res):
    ...
    sns.violinplot(
        ...
        color="white",
        hue="gender",
        hue_order=sorted(set(res["gender"])),
        split=len(set(res["gender"])) == 2,
    ...
    sns.swarmplot(
        ...
        y="score",
        hue="race-alignment",
        hue_order=sorted(set(res["race-alignment"])),
        order=sorted(set(res["role"])),
        data=res,
        ax=ax,
        palette=palette,
