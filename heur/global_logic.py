    ...
    def _split(self, items, forced_items, weight_capacity):
        ...
        def add_item(item, count=None, to_bag=False):
            ...
            if remaining_weight < 0 or (
                ret is ret_inv and len(ret) >= ItemPriority.MAX_NUMBER_OF_ITEMS
            ):  # TODO: coin slot
                return
            ...
            ret[item] = min(item.count, how_many_already_total + max_to_add) - (
                how_many_already_total - how_many_already
            )
        ...
        for allow_unknown_status in [False, True]:
            item = self.agent.inventory.get_best_melee_weapon(
                items=forced_items + items, allow_unknown_status=allow_unknown_status
            )
            if item is not None:
                add_item(item)

            for item in self.agent.inventory.get_best_armorset(
                items=forced_items + items, allow_unknown_status=allow_unknown_status
            ):
                if item is not None:
                    add_item(item)
        ...
        for item in items:
            if item.is_unambiguous():
                ...
                if self.agent.character.role in [
                    Character.RANGER,
                    Character.ROGUE,
                    Character.SAMURAI,
                    Character.TOURIST,
                ] and (item.is_launcher() or item.is_fired_projectile()):
                    add_item(item)

        if self.agent.character.alignment == Character.LAWFUL:
            for item in sorted(
                filter(lambda i: i.objs[0].name == "long sword", items),
        ...
        for item in sorted(
            filter(lambda i: i.is_thrown_projectile(), items),
            key=lambda i: -utils.calc_dps(*self.agent.character.get_ranged_bonus(None, i)),
        ):
            add_item(item)

        for item in sorted(
            filter(lambda i: i.is_food() and not i.is_corpse(), items),
        ...
        for item in sorted(items, key=lambda i: i.unit_weight(with_content=False)):
            if item.category in [
                ...
                if (
                    not isinstance(item.objs[0], O.Container) or not item.is_chest()
                ) and not item.is_possible_container():
                    to_bag = (
                        O.from_name("cancellation", nh.WAND_CLASS) not in item.objs
                        and not item.is_offensive_usable_wand()
                    )
                    to_bag = False
...
class GlobalLogic:
    ...
    def update(self):
        if not self.agent.character.prop.hallu:
            ...
            if (
                ...
                and utils.isin(self.agent.glyphs, G.SHOPKEEPER).any()
            ):
    ...
    def solve_sokoban_strategy(self):
        ...
        def push_bolder(ty, tx, dy, dx):
            while 1:
                ...
                if (self.agent.blstats.y, self.agent.blstats.x) == (ty, tx):
                    assert (
                        "You hear a monster behind the boulder." in message
                        or "You try to move the boulder, but in vain." in message
                    ), message
                    if self.agent.bfs()[ty + dy, tx + dx] != -1:
                        ...
                        for item in flatten_items(self.agent.inventory.items):
                            if item.is_unambiguous() and item.object in [
                                O.from_name("pick-axe"),
                                O.from_name("dwarvish mattock"),
                            ]:
                                pickaxe = item
                                break
        ...
        while 1:
            ...
            for smap, answer in soko_solver.maps.items():
                ...
                offset = np.array(min(zip(*wall_map.nonzero()))) - np.array(
                    min(zip(*(sokomap.sokomap == soko_solver.WALL).nonzero()))
                )
                mask = wall_map[
                    offset[0] : offset[0] + sokomap.sokomap.shape[0], offset[1] : offset[1] + sokomap.sokomap.shape[1]
                ]
            ...
            for (y, x), (dy, dx) in answer:
                ...
                if (
                    self.agent.bfs()[ty, tx] != -1
                    and ((soko_boulder_mask | mask) == soko_boulder_mask).all()
                    and self.agent.glyphs[ty + dy, tx + dx] in G.BOULDER
                ):
                    ...
                    with self.agent.env.debug_log("checking for monsters"):
                        ...
                        to_visit_mask[
                            offset[0] : offset[0] + sokomap.sokomap.shape[0],
    ...
    def wait_out_unexpected_state_strategy(self):
        ...
        while (
            ...
            or self.agent.character.prop.confusion
            or self.agent.character.prop.stun
            or self.agent.character.prop.hallu
            or self.agent.character.prop.polymorph
        ):
            if not yielded:
                yield True
                yielded = True
    ...
    def can_sacrify(self, item):
        ...
        if (
            (mname == "pony" and self.agent.character.role in [Character.KNIGHT, Character.BARBARIAN])
            or (mname == "kitten" and self.agent.character.role == [Character.BARBARIAN, Character.WIZARD])
            or (mname == "little dog" and item.naming)
        ):  # little dogs are always named
    ...
    def offer_corpses(self):
        ...
        altars = [
            p
            for p, alignment in self.agent.current_level().altars.items()
                ...
                for item in flatten_items(self.agent.inventory.items):
                    if self.can_sacrify(item):
                        with self.agent.atom_operation():
                            ...
                            assert (
                                "Your sacrifice is consumed in a flash of light" in self.agent.message
                                or "Your sacrifice is consumed in a burst of flame" in self.agent.message
                                or (
                                    "The blood covers the altar!" in self.agent.message
                                    and "You have summoned " in self.agent.message
                                )
                            ), self.agent.message
                            break
                else:
        ...
        while 1:
            ...
            def go_to_strategy(y, x):
                return (
                    ...
                    .preempt(
                        self.agent,
                        [
                            self.agent.exploration.go_to_strategy(y, x)
                            .preempt(
                                self.agent,
                                [
                                    ...
                                    self.dip_for_excalibur().condition(
                                        lambda: self.agent.blstats.experience_level >= 7
                                    ),
                                ],
                            )
                            .condition(
                                lambda: self._got_artifact
                                or not any(
                                    [
                                        alignment == self.agent.character.alignment
            ...
            (
                ...
                .preempt(
                    self.agent,
                    [
                        self.agent.exploration.explore_stairs(go_to_strategy, all=True).condition(
                            explore_stairs_condition
                        ),
                    ],
                )
    ...
    def global_strategy(self):
        return (
            ...
            .preempt(
                self.agent,
                [
                    self.solve_sokoban_strategy().condition(
                        lambda: self.milestone == Milestone.SOLVE_SOKOBAN
                        and self.agent.current_level().dungeon_number == Level.SOKOBAN
                    )
                ],
            )
            .preempt(
                self.agent,
                [
                    self.offer_corpses().preempt(
                        self.agent,
                        [
                            self.agent.eat_corpses_from_ground().condition(
                                lambda: self.agent.blstats.hunger_state >= Hunger.NOT_HUNGRY
                            ),
                        ],
                    ),
                ],
            ...
            .preempt(
                self.agent,
                [
                    self.agent.eat_corpses_from_ground(only_below_me=True).condition(
                        lambda: self.agent.blstats.hunger_state >= Hunger.NOT_HUNGRY
                    ),
                    self.agent.eat_corpses_from_ground()
                    .every(5)
