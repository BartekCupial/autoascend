...
Armor = namedtuple(
    "Armor", "name,desc,kn,mgc,blk,power,prob,delay,wt,cost,ac,can,sub,metal,c".split(",")
...
def HELM(name, desc, kn, mgc, power, prob, delay, wt, cost, ac, can, metal, c):
    return ARMOR(name, desc, kn, mgc, 0, power, prob, delay, wt, cost, ac, can, ARM_HELM, metal, c)
...
def CLOAK(name, desc, kn, mgc, power, prob, delay, wt, cost, ac, can, metal, c):
    return ARMOR(name, desc, kn, mgc, 0, power, prob, delay, wt, cost, ac, can, ARM_CLOAK, metal, c)
...
def SHIELD(name, desc, kn, mgc, blk, power, prob, delay, wt, cost, ac, can, metal, c):
    return ARMOR(name, desc, kn, mgc, blk, power, prob, delay, wt, cost, ac, can, ARM_SHIELD, metal, c)
...
def GLOVES(name, desc, kn, mgc, power, prob, delay, wt, cost, ac, can, metal, c):
    return ARMOR(name, desc, kn, mgc, 0, power, prob, delay, wt, cost, ac, can, ARM_GLOVES, metal, c)
...
def BOOTS(name, desc, kn, mgc, power, prob, delay, wt, cost, ac, can, metal, c):
    return ARMOR(name, desc, kn, mgc, 0, power, prob, delay, wt, cost, ac, can, ARM_BOOTS, metal, c)
...
def DRGN_ARMR(name, mgc, power, cost, ac, color):
    return ARMOR(name, None, 1, mgc, 1, power, 0, 5, 40, cost, ac, 0, ARM_SUIT, DRAGON_HIDE, color)
...
objects = [
    None,
    PROJECTILE("arrow", None, 1, 55, 1, 2, 6, 6, 0, IRON, -P_BOW, HI_METAL),
    PROJECTILE("elven arrow", "runed arrow", 0, 20, 1, 2, 7, 6, 0, WOOD, -P_BOW, HI_WOOD),
    PROJECTILE("orcish arrow", "crude arrow", 0, 20, 1, 2, 5, 6, 0, IRON, -P_BOW, CLR_BLACK),
    PROJECTILE("silver arrow", None, 1, 12, 1, 5, 6, 6, 0, SILVER, -P_BOW, HI_SILVER),
    PROJECTILE("ya", "bamboo arrow", 0, 15, 1, 4, 7, 7, 1, METAL, -P_BOW, HI_METAL),
    PROJECTILE("crossbow bolt", None, 1, 55, 1, 2, 4, 6, 0, IRON, -P_CROSSBOW, HI_METAL),
    # /* missiles that don't use a launcher */
    WEAPON("dart", None, 1, 1, 0, 60, 1, 2, 3, 2, 0, P, -P_DART, IRON, HI_METAL),
    WEAPON("shuriken", "throwing star", 0, 1, 0, 35, 1, 5, 8, 6, 2, P, -P_SHURIKEN, IRON, HI_METAL),
    WEAPON("boomerang", None, 1, 1, 0, 15, 5, 20, 9, 9, 0, 0, -P_BOOMERANG, WOOD, HI_WOOD),
    ...
    WEAPON("spear", None, 1, 1, 0, 50, 30, 3, 6, 8, 0, P, P_SPEAR, IRON, HI_METAL),
    ...
    WEAPON("dwarvish spear", "stout spear", 0, 1, 0, 12, 35, 3, 8, 8, 0, P, P_SPEAR, IRON, HI_METAL),
    WEAPON("silver spear", None, 1, 1, 0, 2, 36, 40, 6, 8, 0, P, P_SPEAR, SILVER, HI_SILVER),
    WEAPON("javelin", "throwing spear", 0, 1, 0, 10, 20, 3, 6, 6, 0, P, P_SPEAR, IRON, HI_METAL),
    # /* spearish; doesn't stack, not intended to be thrown */
    WEAPON("trident", None, 1, 0, 0, 8, 25, 5, 6, 4, 0, P, P_TRIDENT, IRON, HI_METAL),
    # /* +1 small, +2d4 large */
    # /* blades; all stack */
    WEAPON("dagger", None, 1, 1, 0, 30, 10, 4, 4, 3, 2, P, P_DAGGER, IRON, HI_METAL),
    WEAPON("elven dagger", "runed dagger", 0, 1, 0, 10, 10, 4, 5, 3, 2, P, P_DAGGER, WOOD, HI_WOOD),
    WEAPON("orcish dagger", "crude dagger", 0, 1, 0, 12, 10, 4, 3, 3, 2, P, P_DAGGER, IRON, CLR_BLACK),
    WEAPON("silver dagger", None, 1, 1, 0, 3, 12, 40, 4, 3, 2, P, P_DAGGER, SILVER, HI_SILVER),
    WEAPON("athame", None, 1, 1, 0, 0, 10, 4, 4, 3, 2, S, P_DAGGER, IRON, HI_METAL),
    WEAPON("scalpel", None, 1, 1, 0, 0, 5, 6, 3, 3, 2, S, P_KNIFE, METAL, HI_METAL),
    WEAPON("knife", None, 1, 1, 0, 20, 5, 4, 3, 2, 0, P | S, P_KNIFE, IRON, HI_METAL),
    WEAPON("stiletto", None, 1, 1, 0, 5, 5, 4, 3, 2, 0, P | S, P_KNIFE, IRON, HI_METAL),
    ...
    WEAPON("worm tooth", None, 1, 1, 0, 0, 20, 2, 2, 2, 0, 0, P_KNIFE, 0, CLR_WHITE),
    WEAPON("crysknife", None, 1, 1, 0, 0, 20, 100, 10, 10, 3, P, P_KNIFE, MINERAL, CLR_WHITE),
    # /* axes */
    WEAPON("axe", None, 1, 0, 0, 40, 60, 8, 6, 4, 0, S, P_AXE, IRON, HI_METAL),
    WEAPON(
        "battle-axe",
        "double-headed axe",  # /* "double-bitted"? */
        0,
        0,
        1,
        10,
    ...
    # /* swords */
    WEAPON("short sword", None, 1, 0, 0, 8, 30, 10, 6, 8, 0, P, P_SHORT_SWORD, IRON, HI_METAL),
    WEAPON("elven short sword", "runed short sword", 0, 0, 0, 2, 30, 10, 8, 8, 0, P, P_SHORT_SWORD, WOOD, HI_WOOD),
    WEAPON("orcish short sword", "crude short sword", 0, 0, 0, 3, 30, 10, 5, 8, 0, P, P_SHORT_SWORD, IRON, CLR_BLACK),
    WEAPON("dwarvish short sword", "broad short sword", 0, 0, 0, 2, 30, 10, 7, 8, 0, P, P_SHORT_SWORD, IRON, HI_METAL),
    WEAPON("scimitar", "curved sword", 0, 0, 0, 15, 40, 15, 8, 8, 0, S, P_SCIMITAR, IRON, HI_METAL),
    WEAPON("silver saber", None, 1, 0, 0, 6, 40, 75, 8, 8, 0, S, P_SABER, SILVER, HI_SILVER),
    WEAPON("broadsword", None, 1, 0, 0, 8, 70, 10, 4, 6, 0, S, P_BROAD_SWORD, IRON, HI_METAL),
    # /* +d4 small, +1 large */
    WEAPON("elven broadsword", "runed broadsword", 0, 0, 0, 4, 70, 10, 6, 6, 0, S, P_BROAD_SWORD, WOOD, HI_WOOD),
    # /* +d4 small, +1 large */
    WEAPON("long sword", None, 1, 0, 0, 50, 40, 15, 8, 12, 0, S, P_LONG_SWORD, IRON, HI_METAL),
    WEAPON("two-handed sword", None, 1, 0, 1, 22, 150, 50, 12, 6, 0, S, P_TWO_HANDED_SWORD, IRON, HI_METAL),
    # /* +2d6 large */
    WEAPON("katana", "samurai sword", 0, 0, 0, 4, 40, 80, 10, 12, 1, S, P_LONG_SWORD, IRON, HI_METAL),
    # /* special swords set up for artifacts */
    WEAPON("tsurugi", "long samurai sword", 0, 0, 1, 0, 60, 500, 16, 8, 2, S, P_TWO_HANDED_SWORD, METAL, HI_METAL),
    # /* +2d6 large */
    WEAPON("runesword", "runed broadsword", 0, 0, 0, 0, 40, 300, 4, 6, 0, S, P_BROAD_SWORD, IRON, CLR_BLACK),
    # /* +d4 small, +1 large; Stormbringer: +5d2 +d8 from level drain */
    # /* polearms */
    # /* spear-type */
    WEAPON("partisan", "vulgar polearm", 0, 0, 1, 5, 80, 10, 6, 6, 0, P, P_POLEARMS, IRON, HI_METAL),
    # /* +1 large */
    WEAPON("ranseur", "hilted polearm", 0, 0, 1, 5, 50, 6, 4, 4, 0, P, P_POLEARMS, IRON, HI_METAL),
    # /* +d4 both */
    WEAPON("spetum", "forked polearm", 0, 0, 1, 5, 50, 5, 6, 6, 0, P, P_POLEARMS, IRON, HI_METAL),
    # /* +1 small, +d6 large */
    WEAPON("glaive", "single-edged polearm", 0, 0, 1, 8, 75, 6, 6, 10, 0, S, P_POLEARMS, IRON, HI_METAL),
    WEAPON("lance", None, 1, 0, 0, 4, 180, 10, 6, 8, 0, P, P_LANCE, IRON, HI_METAL),
    # /* +2d10 when jousting with lance as primary weapon */
    # /* axe-type */
    WEAPON("halberd", "angled poleaxe", 0, 0, 1, 8, 150, 10, 10, 6, 0, P | S, P_POLEARMS, IRON, HI_METAL),
    # /* +1d6 large */
    WEAPON("bardiche", "long poleaxe", 0, 0, 1, 4, 120, 7, 4, 4, 0, S, P_POLEARMS, IRON, HI_METAL),
    # /* +1d4 small, +2d4 large */
    WEAPON("voulge", "pole cleaver", 0, 0, 1, 4, 125, 5, 4, 4, 0, S, P_POLEARMS, IRON, HI_METAL),
    # /* +d4 both */
    WEAPON("dwarvish mattock", "broad pick", 0, 0, 1, 13, 120, 50, 12, 8, -1, B, P_PICK_AXE, IRON, HI_METAL),
    # /* curved/hooked */
    WEAPON("fauchard", "pole sickle", 0, 0, 1, 6, 60, 5, 6, 8, 0, P | S, P_POLEARMS, IRON, HI_METAL),
    WEAPON("guisarme", "pruning hook", 0, 0, 1, 6, 80, 5, 4, 8, 0, S, P_POLEARMS, IRON, HI_METAL),
    # /* +1d4 small */
    WEAPON("bill-guisarme", "hooked polearm", 0, 0, 1, 4, 120, 7, 4, 10, 0, P | S, P_POLEARMS, IRON, HI_METAL),
    # /* +1d4 small */
    # /* other */
    WEAPON("lucern hammer", "pronged polearm", 0, 0, 1, 5, 150, 7, 4, 6, 0, B | P, P_POLEARMS, IRON, HI_METAL),
    # /* +1d4 small */
    WEAPON("bec de corbin", "beaked polearm", 0, 0, 1, 4, 100, 8, 8, 6, 0, B | P, P_POLEARMS, IRON, HI_METAL),
    # /* bludgeons */
    WEAPON("mace", None, 1, 0, 0, 40, 30, 5, 6, 6, 0, B, P_MACE, IRON, HI_METAL),
    # /* +1 small */
    WEAPON("morning star", None, 1, 0, 0, 12, 120, 10, 4, 6, 0, B, P_MORNING_STAR, IRON, HI_METAL),
    # /* +d4 small, +1 large */
    WEAPON("war hammer", None, 1, 0, 0, 15, 50, 5, 4, 4, 0, B, P_HAMMER, IRON, HI_METAL),
    # /* +1 small */
    WEAPON("club", None, 1, 0, 0, 12, 30, 3, 6, 3, 0, B, P_CLUB, WOOD, HI_WOOD),
    WEAPON("rubber hose", None, 1, 0, 0, 0, 20, 3, 4, 3, 0, B, P_WHIP, PLASTIC, CLR_BROWN),
    WEAPON("quarterstaff", "staff", 0, 0, 1, 11, 40, 5, 6, 6, 0, B, P_QUARTERSTAFF, WOOD, HI_WOOD),
    # /* two-piece */
    WEAPON("aklys", "thonged club", 0, 0, 0, 8, 15, 4, 6, 3, 0, B, P_CLUB, IRON, HI_METAL),
    WEAPON("flail", None, 1, 0, 0, 40, 15, 4, 6, 4, 0, B, P_FLAIL, IRON, HI_METAL),
    # /* +1 small, +1d4 large */
    # /* misc */
    WEAPON("bullwhip", None, 1, 0, 0, 2, 20, 4, 2, 1, 0, 0, P_WHIP, LEATHER, CLR_BROWN),
    # /* bows */
    ...
    HELM("orcish helm", "iron skull cap", 0, 0, 0, 6, 1, 30, 10, 9, 0, IRON, CLR_BLACK),
    HELM("dwarvish iron helm", "hard hat", 0, 0, 0, 6, 1, 40, 20, 8, 0, IRON, HI_METAL),
    HELM("fedora", None, 1, 0, 0, 0, 0, 3, 1, 10, 0, CLOTH, CLR_BROWN),
    HELM("cornuthaum", "conical hat", 0, 1, CLAIRVOYANT, 3, 1, 4, 80, 10, 1, CLOTH, CLR_BLUE),
    ...
    HELM("dented pot", None, 1, 0, 0, 2, 0, 10, 8, 9, 0, IRON, CLR_BLACK),
    # /* with shuffled appearances... */
    HELM("helmet", "plumed helmet", 0, 0, 0, 10, 1, 30, 10, 9, 0, IRON, HI_METAL),
    HELM("helm of brilliance", "etched helmet", 0, 1, 0, 6, 1, 50, 50, 9, 0, IRON, CLR_GREEN),
    HELM("helm of opposite alignment", "crested helmet", 0, 1, 0, 6, 1, 50, 50, 9, 0, IRON, HI_METAL),
    HELM("helm of telepathy", "visored helmet", 0, 1, TELEPAT, 2, 1, 50, 50, 9, 0, IRON, HI_METAL),
    # /* suits of armor */
    # /*
    ...
    # undef DRGN_ARMR
    # /* other suits */
    ARMOR("plate mail", None, 1, 0, 1, 0, 44, 5, 450, 600, 3, 2, ARM_SUIT, IRON, HI_METAL),
    ARMOR("crystal plate mail", None, 1, 0, 1, 0, 10, 5, 450, 820, 3, 2, ARM_SUIT, GLASS, CLR_WHITE),
    ARMOR("bronze plate mail", None, 1, 0, 1, 0, 25, 5, 450, 400, 4, 1, ARM_SUIT, COPPER, HI_COPPER),
    ARMOR("splint mail", None, 1, 0, 1, 0, 62, 5, 400, 80, 4, 1, ARM_SUIT, IRON, HI_METAL),
    ARMOR("banded mail", None, 1, 0, 1, 0, 72, 5, 350, 90, 4, 1, ARM_SUIT, IRON, HI_METAL),
    ARMOR("dwarvish mithril-coat", None, 1, 0, 0, 0, 10, 1, 150, 240, 4, 2, ARM_SUIT, MITHRIL, HI_SILVER),
    ARMOR("elven mithril-coat", None, 1, 0, 0, 0, 15, 1, 150, 240, 5, 2, ARM_SUIT, MITHRIL, HI_SILVER),
    ARMOR("chain mail", None, 1, 0, 0, 0, 72, 5, 300, 75, 5, 1, ARM_SUIT, IRON, HI_METAL),
    ARMOR("orcish chain mail", "crude chain mail", 0, 0, 0, 0, 20, 5, 300, 75, 6, 1, ARM_SUIT, IRON, CLR_BLACK),
    ARMOR("scale mail", None, 1, 0, 0, 0, 72, 5, 250, 45, 6, 1, ARM_SUIT, IRON, HI_METAL),
    ARMOR("studded leather armor", None, 1, 0, 0, 0, 72, 3, 200, 15, 7, 1, ARM_SUIT, LEATHER, HI_LEATHER),
    ARMOR("ring mail", None, 1, 0, 0, 0, 72, 5, 250, 100, 7, 1, ARM_SUIT, IRON, HI_METAL),
    ARMOR("orcish ring mail", "crude ring mail", 0, 0, 0, 0, 20, 5, 250, 80, 8, 1, ARM_SUIT, IRON, CLR_BLACK),
    ARMOR("leather armor", None, 1, 0, 0, 0, 82, 3, 150, 5, 8, 1, ARM_SUIT, LEATHER, HI_LEATHER),
    ARMOR("leather jacket", None, 1, 0, 0, 0, 12, 0, 30, 10, 9, 0, ARM_SUIT, LEATHER, CLR_BLACK),
    # /* shirts */
    ARMOR("Hawaiian shirt", None, 1, 0, 0, 0, 8, 0, 5, 3, 10, 0, ARM_SHIRT, CLOTH, CLR_MAGENTA),
    ARMOR("T-shirt", None, 1, 0, 0, 0, 2, 0, 5, 2, 10, 0, ARM_SHIRT, CLOTH, CLR_WHITE),
    # /* cloaks */
    CLOAK("mummy wrapping", None, 1, 0, 0, 0, 0, 3, 2, 10, 1, CLOTH, CLR_GRAY),
    # /* worn mummy wrapping blocks invisibility */
    CLOAK("elven cloak", "faded pall", 0, 1, STEALTH, 8, 0, 10, 60, 9, 1, CLOTH, CLR_BLACK),
    ...
    CLOAK("robe", None, 1, 1, 0, 3, 0, 15, 50, 8, 2, CLOTH, CLR_RED),
    # /* robe was adopted from slash'em, where it's worn as a suit
    # rather than as a cloak and there are several variations */
    CLOAK("alchemy smock", "apron", 0, 1, POISON_RES, 9, 0, 10, 50, 9, 1, CLOTH, CLR_WHITE),
    CLOAK("leather cloak", None, 1, 0, 0, 8, 0, 15, 40, 9, 1, LEATHER, CLR_BROWN),
    # /* with shuffled appearances... */
    CLOAK("cloak of protection", "tattered cape", 0, 1, PROTECTION, 9, 0, 10, 50, 7, 3, CLOTH, HI_CLOTH),
    # /* cloak of protection is now the only item conferring MC 3 */
    CLOAK("cloak of invisibility", "opera cloak", 0, 1, INVIS, 10, 0, 10, 60, 9, 1, CLOTH, CLR_BRIGHT_MAGENTA),
    CLOAK("cloak of magic resistance", "ornamental cope", 0, 1, ANTIMAGIC, 2, 0, 10, 60, 9, 1, CLOTH, CLR_WHITE),
    # /*  'cope' is not a spelling mistake... leave it be */
    CLOAK("cloak of displacement", "piece of cloth", 0, 1, DISPLACED, 10, 0, 10, 50, 9, 1, CLOTH, HI_CLOTH),
    # /* shields */
    SHIELD("small shield", None, 1, 0, 0, 0, 6, 0, 30, 3, 9, 0, WOOD, HI_WOOD),
    SHIELD("elven shield", "blue and green shield", 0, 0, 0, 0, 2, 0, 40, 7, 8, 0, WOOD, CLR_GREEN),
    ...
    SHIELD("large shield", None, 1, 0, 1, 0, 7, 0, 100, 10, 8, 0, IRON, HI_METAL),
    SHIELD("dwarvish roundshield", "large round shield", 0, 0, 0, 0, 4, 0, 100, 10, 8, 0, IRON, HI_METAL),
    SHIELD(
        "shield of reflection", "polished silver shield", 0, 1, 0, REFLECTING, 3, 0, 50, 50, 8, 0, SILVER, HI_SILVER
    ),
    # /* gloves */
    ...
    # */
    GLOVES("leather gloves", "old gloves", 0, 0, 0, 16, 1, 10, 8, 9, 0, LEATHER, HI_LEATHER),
    GLOVES("gauntlets of fumbling", "padded gloves", 0, 1, FUMBLING, 8, 1, 10, 50, 9, 0, LEATHER, HI_LEATHER),
    ...
    # /* boots */
    BOOTS("low boots", "walking shoes", 0, 0, 0, 25, 2, 10, 8, 9, 0, LEATHER, HI_LEATHER),
    BOOTS("iron shoes", "hard shoes", 0, 0, 0, 7, 2, 50, 16, 8, 0, IRON, HI_METAL),
    BOOTS("high boots", "jackboots", 0, 0, 0, 15, 2, 20, 12, 8, 0, LEATHER, HI_LEATHER),
    # /* with shuffled appearances... */
    ...
    WEPTOOL("pick-axe", None, 1, 0, 0, 20, 100, 50, 6, 3, WHACK, P_PICK_AXE, IRON, HI_METAL),
    WEPTOOL("grappling hook", "iron hook", 0, 0, 0, 5, 30, 50, 2, 6, WHACK, P_FLAIL, IRON, HI_METAL),
    WEPTOOL("unicorn horn", None, 1, 1, 1, 0, 20, 100, 12, 12, PIERCE, P_UNICORN_HORN, BONE, CLR_WHITE),
    ...
    SPELL_LEATHER("dig", "parchment", P_MATTER_SPELL, 20, 6, 5, 1, RAY, HI_LEATHER),
    SPELL_LEATHER("magic missile", "vellum", P_ATTACK_SPELL, 45, 2, 2, 1, RAY, HI_LEATHER),
...
def possibilities_from_glyph(i):
    ...
    if cat in [nh.TOOL_CLASS, nh.FOOD_CLASS]:
        return [
            ...
            if o is not None and ord(nh.objclass(i).oc_class) == cat and (o.desc or o.name) == desc
        ]
...
def possible_glyphs_from_object(obj):
    return [
        i
        for i in range(nh.GLYPH_OBJ_OFF, nh.GLYPH_OBJ_OFF + nh.NUM_OBJECTS)
...
def from_name(name, category=None):
    ret = []
    for i, o in enumerate(objects):
        if o is not None and o.name == name and (category is None or ord(nh.objclass(i).oc_class) == category):
            ret.append(o)
